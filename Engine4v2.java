...
public class Engine4 extends Canvas implements Runnable{
				...
				if (level == -2) {				
					if (countdownMethod()) {
						if (!isGameStarting) {
							...
							//level = 2;
							level = lm.getLevel();
							System.out.println("We are on LVL: "+level);
	...
	public void restartingPlayersPositions() {
		//player2.setPosY(520);
		player1.resetPositions(30, 550);
		player2.resetPositions(540, 550);
	...
	public void paint (Graphics g) {	
		...
		if (level == -2) {
			...
			f = f.deriveFont(50f);
			bufferGraphics.setFont(f);
			bufferGraphics.drawString("LOADING  GAME . . .", 100, 250);
			g.drawImage(offScreen, 0, 0, this);
		}
